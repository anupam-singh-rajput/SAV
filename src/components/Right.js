...
<div className="input" style={{display: 'flex',width: '37%'}}>
      ...
      <div className='RightsortingBars' style={{position:"relative"}}>
        {primaryArray &&
          primaryArray.map((val, key) => {
            return (
              <div
                ...
                id={key}
                key={key}
                style={{ height: val , width: "50px"}}
              >{val}</div>
            )
          })}
      </div>
      {algorithm.name !== undefined && (
        <div className='algoInfo' style={{width:"40%",position:"absolute",left:"55%",bottom:"20px"}}>
